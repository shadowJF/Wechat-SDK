...
import com.yonyou.nccloud.weixin.sdk.msg.out.OutCustomMsg;
import com.yonyou.nccloud.weixin.sdk.msg.out.OutTextMsg;
import com.yonyou.nccloud.weixin.sdk.sup.WechatMsgSupport;


/**
 * 将此 DemoController 在YourJFinalConfig 中注册路由， 并设置好weixin开发者中心的 URL 与 token ，使 URL 指向该 DemoController
 * 继承自父类 WeixinController 的 index 方法即可直接运行看效果，在此基础之上修改相关的方法即可进行实际项目开发
 */
public class WeixinMsgController extends WechatMsgSupport {

  static Logger logger = LoggerFactory.getLogger(WeixinMsgController.class);
  private static final String helpStr =
      "\t发送 help 可获得帮助，发送\"视频\" 可获取视频教程，发送 \"美女\" 可看美女，发送 music 可听音乐 ，发送新闻可看JFinal新版本消息。公众号功能持续完善中";
  ...
  // outMsg.addNews("JFinal 1.6 发布,JAVA 极速 WEB+ORM 框架",
  ...
  // * 实现父类抽方法，处理接收语音识别结果
  // */
  // protected void processInSpeechRecognitionResults(InSpeechRecognitionResults
  ...
  // }
  // @Override
  // protected void processInShakearoundUserShakeEvent(InShakearoundUserShakeEvent
