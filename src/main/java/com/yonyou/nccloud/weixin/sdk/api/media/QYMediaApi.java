package com.yonyou.nccloud.weixin.sdk.api.media;
 ...
 * 
 * @author Javen
 *         文档：http://qydev.weixin.qq.com/wiki/index.php?title=%E4%B8%8A%E4%BC%A0
 *         %E4%B8%B4%E6%97%B6%E7%B4%A0%E6%9D%90%E6%96%87%E4%BB%B6
 */
public class QYMediaApi {

	/**
	 * 上传的临时多媒体文件有格式 用于上传图片、语音、视频等媒体资源文件以及普通文件（如doc，ppt）
	 */
	...
	public static ApiResult uploadMedia(MediaType mediaType, File file) {
		String url = upload_url + AccessTokenApi.getQYAccessTokenStr() + "&type=" + mediaType.get();
	...
	public static MediaFile getMedia(String media_id) {
		String url = get_url + AccessTokenApi.getQYAccessTokenStr() + "&media_id=" + media_id;
	...
	public static ApiResult addNews(MediaMpNews mediaMpNews) {
		String url = add_news_url + AccessTokenApi.getQYAccessTokenStr();

		String jsonResult = HttpKit.post(url, JSON.toJSONString(mediaMpNews));
	...
	public static ApiResult addMaterial(File file, String agentid, MediaType type) {
		String url = addMaterialUrl.replace("AGENTID", agentid).replace("TYPE", type.get()).replace("ACCESS_TOKEN",
				AccessTokenApi.getQYAccessTokenStr());
	...
	public static ApiResult delMaterial(String media_id, String agentid) {
		String url = del_material_url.replace("AGENTID", agentid).replace("MEDIA_ID", media_id).replace("ACCESS_TOKEN",
				AccessTokenApi.getQYAccessTokenStr());
	...
	public static ApiResult updateNews(MediaMpNews mediaMpNews) {
		String url = update_news_url + AccessTokenApi.getQYAccessTokenStr();
		String jsonResult = HttpKit.post(url, JSON.toJSONString(mediaMpNews));
		return new ApiResult(jsonResult);
	}
	...
	public static ApiResult getMaterialCount(String agentid) {
		String url = get_materialcount_url.replace("ACCESS_TOKEN", AccessTokenApi.getQYAccessTokenStr())
	...
	public static ApiResult batchGetMaterial(MediaType mediaType, int offset, int count, int agentid) {
		String url = batchget_material_url + AccessTokenApi.getQYAccessTokenStr();
