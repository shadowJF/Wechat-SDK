...
public class QYWechatApiSupport {
   ...
   * 发送文本消息
   */
  public static void sendTextMssage(HttpServletRequest request, HttpServletResponse response,
      QiYeTextMsg text) {
    try {
   ...
   * 发送图片
   */
  public static void sendImage(HttpServletRequest request, HttpServletResponse response,
      QiYeImageMsg image) {
    try {
      QYApiConfigKit.setThreadLocalApiConfig(getApiConfig(request));
  ...
  public static void creatMenuApi(HttpServletRequest request, HttpServletResponse response) {
    try {
      ...
      HttpKit.renderText(response,
          (QYMenuApi.createMenu(JSON.toJSONString(menu), agentid).getJson()));
    } finally {
      QYApiConfigKit.removeThreadLocalApiConfig();
    }
  ...
  /**
   * 获取指定部分列表 {"errcode":0,"errmsg":"ok","department":[{"id":1,"name":"企业号体验34797078","
   ...
   */
  public static void getDepartmentUserSimpleList(HttpServletRequest request,
    ...
    try {
      ...
      ApiResult apiResult =
          QYConUserApi.getDepartmentUserSimpleList(department_id, fetch_child, status);
      HttpKit.renderText(response, apiResult.getJson());
    } finally {
      QYApiConfigKit.removeThreadLocalApiConfig();
    }
  ...
  /**
   * 增加标签成员 {"data": "{\"tagid\": \"1\"," + "\"userlist\":[ \"Javen\",\"lisi\"]," +
   * "\"partylist\": []}"}
   */
  ...
  /**
   * 删除标签成员{"data": "{\"tagid\": \"1\"," + "\"userlist\":[ \"Javen\",\"lisi\"]," +
   * "\"partylist\": []}"}
   */
  ...
  /**
   * 邀请成员关注{"data": "{" + "\"touser\":\"lisi|Javen\"," + "\"toparty\":\"3\"," + "\"totag\":\"1\"," +
  ...
  /**
   * ApiResult apiResult = MediaApi.uploadMedia(MediaType.IMAGE, file); String json =
   * apiResult.getJson(); String mediaId = JSON.parseObject(json).getString("media_id");
   * QiYeImageMsg image = new QiYeImageMsg(mediaId); image.setAgentid("22"); image.setSafe("0");
   * image.setTouser("Javen");
   */
  public static void uploadImage(HttpServletRequest request, HttpServletResponse response,
      File file, QiYeImageMsg image) {
    try {
      QYApiConfigKit.setThreadLocalApiConfig(getApiConfig(request));
  ...
  /**
   * 设置企业号应用{"data":"{" + "\"agentid\": \"22\"," + "\"report_location_flag\": \"1\"," + //
   * "\"logo_mediaid\": \"xxxxx\","+ "\"name\": \"智慧云端日记\"," + "\"description\": \"企业号测试应用\"," +
   * "\"redirect_domain\": \"javen.ngrok.natapp.cn\"," + "\"isreportuser\":1," +
   * "\"isreportenter\":1}";}
   */
  ...
  /**
   * 如果用户未关注将无法转化 openid转换成userid接口{"data":"{\"openid\":\"oD3e5jpSC3C8Qq5uon_SEeRwc9AA\"}"}
   */
