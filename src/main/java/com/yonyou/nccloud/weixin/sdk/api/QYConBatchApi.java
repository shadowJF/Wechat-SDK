...
/**
 * @author Javen 2015年12月20日 管理通讯录>>异步任务接口
 * 
 *         此方法还可以优化 只有url 以及参数（json）不一样
 */
public class QYConBatchApi {
	/**
	 * 邀请成员关注
	 */
	 ...
	 *            "callback":
	 *            回调信息。如填写该项则任务完成后，通过callback推送事件给企业。具体请参考应用回调模式中的相应选项<br/>
	 *            {<br/>
	 *            "url": "xxx",<br/>
	 *            "token": "xxx",<br/>
	...
	public static ApiResult inviteUsers(String data) {
		String jsonStr = HttpUtils.post(batchInviteUsersUrl.replace("ACCESS_TOKEN", AccessTokenApi.getQYAccessTokenStr()),
				data);
	...
	public static ApiResult updateSyncUser(String data) {
		String jsonStr = HttpUtils.post(batchSyncUserUrl.replace("ACCESS_TOKEN", AccessTokenApi.getQYAccessTokenStr()),
				data);
		return new ApiResult(jsonStr);
	}
	 ...
	 * 
	 * @param data
	 *            { "media_id":"xxxxxx", "callback": { "url": "xxx", "token":
	 *            "xxx", "encodingaeskey": "xxx" } }
	 * @return
	 */
	public static ApiResult batgReplaceUser(String data) {
		String jsonStr = HttpUtils.post(batchReplaceUserUrl.replace("ACCESS_TOKEN", AccessTokenApi.getQYAccessTokenStr()),
				data);
		return new ApiResult(jsonStr);
	}
	...
	public static ApiResult batchReplaceParty(String data) {
		String jsonStr = HttpUtils
				.post(batchReplacePartyUrl.replace("ACCESS_TOKEN", AccessTokenApi.getQYAccessTokenStr()), data);
		return new ApiResult(jsonStr);
	}
	...
	public static ApiResult batchGetResult(String jobId) {
		String jsonStr = HttpUtils.get(
				batchGetResultUrl.replace("ACCESS_TOKEN", AccessTokenApi.getQYAccessTokenStr()).replace("JOBID", jobId));
		return new ApiResult(jsonStr);
	}
}
