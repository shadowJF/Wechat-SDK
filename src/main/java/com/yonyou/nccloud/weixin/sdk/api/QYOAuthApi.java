...
public class QYOAuthApi {
  ...
  public static String getCodeUrl(String redirectUri, String state) {
    getCodeUrl =
        getCodeUrl + QYApiConfigKit.getApiConfig().getCorpId() + "&redirect_uri=" + redirectUri
  ...
  public static ApiResult getUserInfoByCode(String code) {
    ...
    String jsonResult =
        HttpUtils.get(getUserInfoUrl + AccessTokenApi.getAccessTokenStr() + "&code=" + code);
   ...
   * openid转换成userid接口
   * 
   * @param data { "openid": "oDOGms-6yCnGrRovBj2yHij5JL6E" }
   * @return
   */
