...
/**
 * 
 * @author Javen 2015年12月27日 管理企业号应用
 */
public class AgentApi {
	 ...
	 * 
	 * @param agentid
	 * @return 该API用于获取企业号某个应用的基本信息，包括头像、<br/>
	 *         昵称、帐号类型、认证类型、可见范围等信息
	 */
	 ...
	 *            "report_location_flag": "0", 企业应用是否打开地理位置上报
	 *            0：不上报；1：进入会话上报；2：持续上报<br/>
	 ...
	 *            }
	 * @return 该API用于设置企业应用的选项设置信息，<br/>
	 *         如：地理位置上报等。第三方服务商不能调用该接口设置授权的主页型应用。
	 */
	public static ApiResult setAgent(String data) {
		String jsonResult = HttpUtils.post(setUrl + AccessTokenApi.getQYAccessTokenStr(), data);
		return new ApiResult(jsonResult);
	}
	 ...
	 * 获取应用概况列表
	 * 
	 * @return 该API 用于获取secret所在管理组内的应用概况，<br/>
	 *         会返回管理组内应用的id及名称、头像等信息
	 */
	public static ApiResult getListAgent() {
		String jsonResult = HttpUtils.get(getListUrl + AccessTokenApi.getQYAccessTokenStr());
		return new ApiResult(jsonResult);
	}
}
