...
/**
 * @author Javen 2015年12月19日 管理通讯录>>管理标签
 */
public class QYConTagApi {
	 ...
	 *            "tagid": id 标签id，整型，指定此参数时新增的标签会生成对应的标签id，<br/>
	 *            不指定时则以目前最大的id自增。 }<br/>
	 * 
	 * @return
	 */
	...
	public static ApiResult deleteTag(String tagid) {
		ParaMap pm = ParaMap.create("access_token", AccessTokenApi.getQYAccessTokenStr()).put("tagid", tagid);
	 ...
	 *            企业成员ID列表，注意：userlist、partylist不能同时为空，单次请求长度不超过1000<br/>
	 *            "partylist": [4]
	 *            企业部门ID列表，注意：userlist、partylist不能同时为空，单次请求长度不超过100<br/>
	 *            }<br/>
	 * @return
	 */
	public static ApiResult addTagUsers(String data) {
		String jsonResult = HttpUtils.post(addTagUsersUrl + AccessTokenApi.getQYAccessTokenStr(), data);
		return new ApiResult(jsonResult);
	}
	 ...
	 *            "userlist":[ "user1","user2"],
	 *            企业部门ID列表，注意：userlist、partylist不能同时为空<br/>
	 *            "partylist":[2,4]<br/>
	 *            }<br/>
	 * @return
	 */
	public static ApiResult deleteTagUsers(String data) {
		String jsonResult = HttpUtils.post(delTagUsersUrl + AccessTokenApi.getQYAccessTokenStr(), data);
		return new ApiResult(jsonResult);
	}
	...
	public static ApiResult getTagList() {
		String jsonResult = HttpUtils.get(getTagListUrl + AccessTokenApi.getQYAccessTokenStr());
		return new ApiResult(jsonResult);
	}
