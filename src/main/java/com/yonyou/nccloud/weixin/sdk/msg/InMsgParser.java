package com.yonyou.nccloud.weixin.sdk.msg;
...
import com.yonyou.nccloud.weixin.sdk.kit.XmlKit;
...
import com.yonyou.nccloud.weixin.sdk.msg.in.event.InShakearoundUserShakeEvent.AroundBeacon;
...
import com.yonyou.nccloud.weixin.sdk.msg.in.event.ScanCodeInfo;
...
public class InMsgParser {
  ...
  /**
   * 消息类型 1：text 文本消息 2：image 图片消息 3：voice 语音消息 4：video 视频消息 shortvideo 小视频消息 5：location 地址位置消息
  ...
  private static InMsg doParse(Document doc) {
    ...
    if ("voice".equals(msgType))
      return parseInVoiceMsgAndInSpeechRecognitionResults(root, toUserName, fromUserName,
          createTime, msgType);
  ...
  private static InMsg parseInTextMsg(Element root, String toUserName, String fromUserName,
  ...
  private static InMsg parseInImageMsg(Element root, String toUserName, String fromUserName,
      Integer createTime, String msgType) {
  ...
  private static InMsg parseInVoiceMsgAndInSpeechRecognitionResults(Element root,
    ...
    if (StringUtils.isBlank(recognition)) {
      ...
      InSpeechRecognitionResults msg =
          new InSpeechRecognitionResults(toUserName, fromUserName, createTime, msgType);
      msg.setMediaId(XmlKit.elementText(root, "MediaId"));
      msg.setFormat(XmlKit.elementText(root, "Format"));
      msg.setMsgId(XmlKit.elementText(root, "MsgId"));
  ...
  private static InMsg parseInVideoMsg(Element root, String toUserName, String fromUserName,
      Integer createTime, String msgType) {
  ...
  private static InMsg parseInLinkMsg(Element root, String toUserName, String fromUserName,
      Integer createTime, String msgType) {
  ...
  // 解析各种事件
  private static InMsg parseInEvent(Element root, String toUserName, String fromUserName,
      Integer createTime, String msgType) {
    ...
    if ("TEMPLATESENDJOBFINISH".equals(event)) {
      InTemplateMsgEvent e =
          new InTemplateMsgEvent(toUserName, fromUserName, createTime, msgType, event);
    ...
    if ("ShakearoundUserShake".equals(event)) {
      InShakearoundUserShakeEvent e =
          new InShakearoundUserShakeEvent(toUserName, fromUserName, createTime, msgType);
      e.setEvent(event);
    ...
    if ("qualification_verify_success".equals(event) || "naming_verify_success".equals(event)
      ...
      InVerifySuccessEvent e =
          new InVerifySuccessEvent(toUserName, fromUserName, createTime, msgType, event);
    ...
    if ("qualification_verify_fail".equals(event) || "naming_verify_fail".equals(event)) {
      InVerifyFailEvent e =
          new InVerifyFailEvent(toUserName, fromUserName, createTime, msgType, event);
    ...
    if ("poi_check_notify".equals(event)) {
      InPoiCheckNotifyEvent e =
          new InPoiCheckNotifyEvent(toUserName, fromUserName, createTime, msgType);
